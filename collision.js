class Collision {
	...
	applyImpulse() {
		if (this.body1.mass + this.body2.mass == 0) {
		...
		for (var i=0; i<this.contacts.length; i++) {
			...
			let invMassSum = this.body1.invMass + this.body2.invMass + distance1CrossNormal * distance1CrossNormal * this.body1.invMomentOfInertia +
								distance2CrossNormal * distance2CrossNormal * this.body2.invMomentOfInertia;
	...
	positionalCorrection() {
		...
		let correction = (Math.max(this.penetration - slack, 0) / (this.body1.invMass + this.body2.invMass)) * percent;
		...
		correctionVector.multiplyScalar(this.body1.invMass);
		correctionVector.negate();
		correctionVector2.multiplyScalar(this.body2.invMass);

		this.body1.translate(correctionVector);
		this.body2.translate(correctionVector2);
	}
